...
The mock data layer lets recruiters and reviewers evaluate the AI's analytical depth, entity extraction, and UI responsiveness — without access to a private Gmail account or exposure of any sensitive communications. The same codebase powers both modes without modification.
...
1. **Fact Extraction** — identifies sender intent and isolates core entities (people, companies, and date references) directly from the raw email body.
2. **Strategic Analysis** — maps extracted facts into strict **Pydantic schemas**, ensuring 100% structural integrity and eliminating LLM schema drift for reliable database storage.
...
- **Multilingual Recognition** — natively understands and categorizes email content in both Czech and English, including complex linguistic nuances.
